...
Here is an approach to use an O-word subroutine to read a local preference file entry and add it as a G-code parameter.
...
;py,PREFFILE = os.path.join(CONFPATH,'machine.pref')
;py,print(PREFFILE)

; get a preference parser instance
;py,Pref = configparser.ConfigParser()
;py,Pref.read(PREFFILE)

; load a preference and print it
;py,this.params['toolToLoad']=Pref.getint('CUSTOM_FORM_ENTRIES', 'Tool to load', fallback=0)
