...
The External E-Stop example shows how to connect external E-Stop inputs
...
- %I0 = Input from the external E-Stop circuit
- %I1 = Input from LinuxCNC's E-Stop
- %I2 = Input from LinuxCNC's E-Stop Reset Pulse
- %I3 = Input from the external reset button
...
[source,hal]
----
# E-Stop example using external components

# The external E-Stop input is normally closed
net ext-estop classicladder.0.in-00 <= external.estop-ok

# Request E-Stop Enable from LinuxCNC
net estop-all-ok iocontrol.0.emc-enable-in <= classicladder.0.out-00

# Request E-Stop Enable from external reset source
net ext-estop-reset classicladder.0.in-03 <= external.estop-reset
...
# This line turns on an external indicator when out of E-Stop
net estop-all-ok => external.estop-status
----
...
E-Stop input before the AXIS E-Stop or the
