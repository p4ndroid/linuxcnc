...
All of these examples assume you are starting with a working AXIS
configuration and have two threads 'base-thread' and 'servo-thread'.
The custom.hal file will be loaded after the configuration HAL file and the
...
For example, connect two input pins from a pendant or another HAL component
...
net button-1 or2.0.in0 <= pendant.button-1
net button-2 or2.0.in1 <= pendant.button-2
net led-1 indicator.led-1 <= or2.0.out
----

When you run this example in an AXIS simulator, you can open a terminal and
see the pins created with 'loadrt or2' by typing in `halcmd show pin or2` in
the terminal.
...
In some cases you might want to run just HAL. For example say you had a
stepper driven device where all you need is to run a stepper motor.
This example uses a HAL signal as the start/stop input.
...
net run-stepgen stepgen.0.enable
sets run-stepgen TRUE
