...
Examples are included for AXIS. These examples demonstrate both 3-axis (XYZ)
cartesian configurations (like mills) and lathe (XZ) setups. Some examples
demonstrate the use of files created for the gcmc (G-code Meta Compiler)
application.
...
[[ngcgui-ini]]
=== Additional INI File items required for NGCGUI

The following INI file items go in the [DISPLAY] section for AXIS
configurations that embed NGCGUI.
