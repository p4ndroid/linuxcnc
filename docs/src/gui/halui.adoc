...
HAL pins, including physical switches, pendant inputs, or pins exported
by a custom userspace HAL component as shown in the example
<<code:Example-HAL-file-connections, Example for MDI_COMMAND connections>>.
...
[source,hal]
----
net quill-up      halui.mdi-command-00 <= pendant.quillup
net reference-pos halui.mdi-command-01 <= pendant.referencepos
net call-mysub    halui.mdi-command-02 <= pendant.callmysub
----
