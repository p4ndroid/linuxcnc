...
just before the AXIS HAL component is marked ready. +
...
[source,python]
----
# The following example assumes HAL buttons exported by user_hal_pins()
# and connected from a pendant or another userspace HAL component.
# button names are 'manual-tab', 'mdi-tab', 'preview-tab', 'numbers-tab'

# for LinuxCNC 2.8+ branch

def user_live_update():
    try:
        if hal.get_value('axisui.manual-tab'):
            root_window.tk.call('.pane.top.tabs','raise','manual')
        elif hal.get_value('axisui.mdi-tab'):
            root_window.tk.call('.pane.top.tabs','raise','mdi')
        elif hal.get_value('axisui.preview-tab'):
            root_window.tk.call('.pane.top.right','raise','preview')
        elif hal.get_value('axisui.numbers-tab'):
