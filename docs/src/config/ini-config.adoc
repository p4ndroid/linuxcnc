...
This reduced tree provides AXIS as the machine-control user interface.

* `DISPLAY = axis` - (string) The file name of the executable providing the user interface to use.
  The retained machine-control display is `axis`.
...
* `DEFAULT_SPINDLE_SPEED = 100` - The default spindle RPM when the spindle is started in manual mode.
  If this setting is not present, this defaults to 1 RPM for AXIS.
...
* `POSTGUI_HALFILE = example2.hal` - Execute 'example2.hal' after the GUI has created its HAL pins.
  AXIS creates HAL pins and supports the use of a postgui halfile to use them.
...
* `INCREMENT = 200` - (real)
  ...
  This setting is effective with AXIS.
