...
After installation it is recommended that you connect the computer
...
straight to one of the supplied AXIS sample configurations for Xylotex
machines.
...
check the drive manual. Note some newer Gecko drives have different timing
than the original one. A https://wiki.linuxcnc.org/[list] is also on the
user maintained LinuxCNC wiki site of more drives.
...
Leave unused physical pins unconnected in the HAL file. They can be connected
later by editing the configuration.
