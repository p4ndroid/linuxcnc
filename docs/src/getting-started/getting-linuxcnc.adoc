...
NOTE: To operate machinery LinuxCNC requires a PREEMPT_RT kernel and the
uspace runtime. For code testing and simulation it is possible to run the
`linuxcnc-uspace` application on a stock distribution kernel.
...
. Determine the device file corresponding to the USB flash drive.
  This information can be found in the output of `sudo dmesg` after
  connecting the device.  `cat /proc/partitions` may also be helpful.
...
At the time of writing the Live Image is available with the preempt-rt kernel
and a matching LinuxCNC.
