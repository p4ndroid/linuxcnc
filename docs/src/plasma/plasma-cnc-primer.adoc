...
Following a number of experimental builds, integrated plasma configurations were
incorporated into LinuxCNC. This reduced tree retains the core motion and HAL
plasma support but does not include a dedicated plasma GUI.
...
== LinuxCNC Plasma Configuration

The retained plasma support is centered on HAL components and ordinary AXIS
configurations. External THC devices such as the Proma 150 can be connected
through HAL, and Mesa THCAD voltage-to-frequency converters can provide torch
voltage feedback through supported Mesa hardware.
