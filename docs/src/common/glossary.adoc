...
kernel-space::
     Code running inside the kernel, as opposed to code running in userspace.
    In this reduced tree, LinuxCNC realtime code runs in userspace on POSIX
    realtime scheduling.
...
Python::
    ...
    for the AXIS GUI and several G-code programming scripts.
...
Real-time::
    (((real-time))) Software that is intended to meet very strict timing deadlines. On Linux, in order to meet these requirements it is necessary to install a PREEMPT_RT kernel and build LinuxCNC for the uspace realtime environment. Realtime software in this tree runs in userspace.

RTAPI::
     (((RTAPI)))A portable interface to real-time operating systems
    using POSIX pthreads with realtime extensions.
