...
cd src                                            <1>
./autogen.sh                                      <2>
./configure --with-realtime=uspace <3>
make clean                                        <4>
make                                              <5>
sudo make setuid                                  <6>
----
...
installed. This reduced tree supports the *uspace* POSIX runtime on
PREEMPT_RT kernels for realtime machine control.
...
Use `--with-realtime=uspace` for PREEMPT_RT or non-realtime simulator
builds.
